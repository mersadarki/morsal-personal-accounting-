...
import { ACCOUNTS, ACCOUNT_LABELS, ACCOUNT_COLORS, COLORS } from '../../lib/constants';
import { Amount, iconBtn } from '../../lib/ui.jsx';

// Distinct icon per account, standing in for a bank logo (we don't embed
// actual bank trademarks) — ملی/اعتبار ملی share Bank Melli's green,
// ویپاد gets Pasargad's warm orange (it's a Pasargad wallet product),
// دلار a currency icon, نقدی a banknote icon. Colors come from the shared
// ACCOUNT_COLORS so the same account reads the same color everywhere
// (balance cards, transaction rows) rather than just here.
const ACCOUNT_ICON = {
  'ملی': Landmark, 'اعتبار ملی': Landmark, 'ویپاد': Wallet, 'نقدی': Banknote, 'دلار': DollarSign,
};
        ...
        {ACCOUNTS.map((a) => {
          ...
          return (
            <div key={a} style={{ background: '#fff', border: `1px solid ${COLORS.line}`, borderRadius: 12, padding: 10, display: 'flex', alignItems: 'center', gap: 9, boxShadow: '0 1px 2px rgba(0,0,0,0.04)' }}>
              <div style={{ width: 34, height: 34, borderRadius: 9, background: ACCOUNT_COLORS[a], color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                <Icon size={17} />
              </div>
